...
const membershipTypes = {
    ...
    config: {
        ...
        validate: {
            query: {
                membershipTypeId: Joi.string().optional().trim(),
                membershipTypeName: Joi.string().optional().trim(),
                enabled: Joi.number().optional().integer().min(0).max(1),
            },
...
const occassions = {
    ...
    config: {
        ...
        validate: {
            query: {
                occassionId: Joi.string().optional().trim(),
                displayNameForOccassion: Joi.string().optional().trim(),
                enabled: Joi.number().optional().integer().min(0).max(1),
            },
            failAction: commonFunctions.failActionFunction
        },
...
module.exports = [membershipTypes, occassions];